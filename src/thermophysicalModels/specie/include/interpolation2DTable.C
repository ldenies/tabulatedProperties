...
template<class Type>
Type Foam::interpolation2DTable<Type>::inverseInterpolate
(
    const List<Tuple2<scalar, Type> >& data,
    const scalar lookupValue
) const
...
) const
{
    ...
    // only 1 column (in X) - interpolate to find Y value
        if (inverse)
        {
            return inverseInterpolate(t.first().second(), valueY);
        }
        else
        {
    ...
    else
    {
        ...
        if (inverse)
        {
                if (x0i == x1i)
                {
                    return inverseInterpolate(t[x0i].second(), valueY);
                }
                else
                {
                    Type y0(inverseInterpolate(t[x0i].second(), valueY));
                    Type y1(inverseInterpolate(t[x1i].second(), valueY));
