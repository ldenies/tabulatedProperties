        ...
        case mtFluidThermo:
        {
            ...
            if (mesh.foundObject<turbulenceModel>("turbulenceModel"))
            {
                const turbulenceModel& turbModel =
					...
					//const scalarField kappaInternal = turbModel.kappaEff();
					//scalarField kappaPatch = turbModel.kappaEff(patchI);
					//int i = 0;
				    //forAll( patch_ , facei)
					//{
						//label celli = patch_.faceCells()[facei];
						//kappaPatch[facei] = kappaInternal[celli];
						//i += 1;
						////Info<<"Cell number "<<celli<<" kappa "<<kappaPatch[facei]<<
						////	" Number: " << i << endl;
					//}
                //return kappaPatch;
