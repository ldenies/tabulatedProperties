...
tmp<scalarField> mutkRoughWallFunctionFvPatchScalarField::calcMut() const
{
    ...
    forAll(mutw, faceI)
    {
		...
		// The avoidance oscillation scheme has been adapted for 
		// supercritical fluids because the viscosity variations are too
		// large

        scalar maxMutw = max(mutw[faceI], muw[faceI]);
        scalar minMutw = min(mutw[faceI], muw[faceI]);
